...
export function newRoute(
  ...
  return {
    player: { x: 0, y: 0 },
    layout: genLayout(cols, rows, packageCount, depotCount, rng, companyCount, boughtCount, lockerPerRow),
...
export function startDay(
  state: GridState,
  opts: { cols: number; rows: number; packageCount: number; depotCount?: number; rng?: () => number; companyCount?: number; boughtCount?: number; lockerPerRow?: number },
): GridState {
  return newRoute(opts.cols, opts.rows, opts.packageCount, opts.depotCount ?? 1, state.routes, opts.rng, opts.companyCount ?? 0, opts.boughtCount ?? 0, opts.lockerPerRow ?? 0);
}
